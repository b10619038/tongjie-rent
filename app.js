...
const APP_STAMP = "2026-08-31-01-13";
const APP_EDIT_COUNT = 287;
...
const CHANGELOG = [
  { ver: APP_STAMP, items: ["發票中文大寫紅字往下0.2公分"] },
...
function invoiceCopyHtml(r, t, copyName) {
  ...
  if (triple) {
    ...
    const cn = hans.map((ch, i) => mk(W, H, cnX[i] - 58, 961, 38, 44, ch, "inv-han")).join("");
  ...
  const cn = hans.map((ch, i) => mk(W, H, cnX[i] - 58, 901, 38, 44, ch, "inv-han")).join("");
  ...
  return `<section class="inv-photo double">
    ...
    ${mk(W, H, 1288, 422, 440, 70, roomNote, "inv-note")}
