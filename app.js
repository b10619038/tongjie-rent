...
const FACTORY_ROSTER_VER = "20260902-1920";
...
const CHANGELOG = [
  ...
  { ver: "2026-09-04-12-59-642", items: ["房間圖示改回原本的床，改成正方形"] },
  ...
  { ver: "2026-08-31-01-47b", items: ["三聯數量一式往左0.3公分"] },
  { ver: "2026-08-31-01-47", items: ["三聯品名紅字往左0.2公分"] },
  { ver: "2026-08-31-01-46", items: ["三聯應稅V往左0.2公分"] },
  { ver: "2026-08-31-01-45", items: ["三聯發票日期紅字往左0.1公分"] },
  { ver: "2026-08-31-01-42", items: ["三聯品名改廠房租金收入，該列紅字往右0.3公分"] },
  ...
  { ver: "2026-08-31-01-34", items: ["手機看發票會等比例縮小，紅字一起縮"] },
  ...
  { ver: "2026-08-31-00-58", items: ["發票地址那行的紅色套印拿掉"] },
  ...
  { ver: "2026-08-30-20-08", items: ["設定新增通知類別、安裝、公司資料與租客密碼"] },
...
function rememberDismiss(ver) {
  const v = String(ver || "");
  if (!v) return;
  try { localStorage.setItem("tj-dismiss-build", v); } catch {}
}
function isNewerBuild(ver) {
  const v = String(ver || "");
  if (!v || v === FILE_VER) return false;
  if (/^\d+$/.test(v) && /^\d+$/.test(FILE_VER)) return Number(v) > Number(FILE_VER);
  return true;
}
...
function closeUpdateNotes() {
  if (ui.pendingFileVer && isNewerBuild(ui.pendingFileVer)) rememberDismiss(ui.pendingFileVer);
  else {
    try { localStorage.setItem("tj-last-ver", APP_VERSION); } catch {}
  }
  hideUpdateUi();
...
function applyAppUpdate() {
  persistLogin();
  persistUi();
  if (!ui.pendingFileVer || !isNewerBuild(ui.pendingFileVer)) {
    markVersionSeen();
    hideUpdateUi();
    return;
  }
...
function flagAppUpdate(fileVer) {
  ...
  if (!isNewerBuild(ver)) return;
  if (isDismissed(ver)) return;
  ui.pendingFileVer = ver;
...
function promptAppUpdate(reg) {
  ui.updateReady = true;
  try { ensureUpdateBar(); } catch {}
  if (lastSeenVersion() === APP_VERSION && !isNewerBuild(ui.pendingFileVer)) return;
  if (canOsNotify()) {
    try {
...
function watchAppUpdate(reg) {
  ...
  const take = () => {
    ...
    try { persistUi(); persistLogin(); } catch {}
...
const COMPANY_BANKS = [
  ...
  { company: "統潔", bank: "鳳山區農會", code: "6190897", account: "00897210912150", holder: "統潔開發有限公司", key: "農會", note: "套房舊客匯款" },
...
function enforceTenantSession() {
  ...
  if (ui.prospectPreview && !(t && t.officialAt && !t.former && !t.cancelledApply)) {
    return;
  }
  if (ui.prospectPreview) {
...
const STUDIO_MONTH_PAY = {
  ...
  "7251": { name: "呂佳芸", paidOn: "", amount: 5000 },
...
const FACTORY_GROUPS = [
  ...
  { group: "牛2", street: "文龍東路", company: "信潔", city: "高雄市鳳山區文龍東路", items: [
    ...
    { no: "牛2-27", unit: "57巷1弄27號", manager: "貴美" },
    { no: "牛2-29", unit: "57巷1弄29號", manager: "貴美" },
    { no: "牛2-31", unit: "57巷1弄31號", manager: "貴美" },
  ...
  { group: "牛6", street: "鳳仁路", company: "信潔", city: "高雄市鳳山區鳳仁路", items: [
    ...
    { no: "牛6-56", unit: "93-56號", manager: "" },
    { no: "牛6-57", unit: "93-57號", manager: "" },
    { no: "牛6-58", unit: "93-58號", manager: "" },
    { no: "牛6-59", unit: "93-59號", manager: "" },
    { no: "牛6-60", unit: "93-60號", manager: "" },
    { no: "牛6-61", unit: "93-61號", manager: "" },
    { no: "牛6-62", unit: "93-62號", manager: "" }
  ]},
...
const FACTORY_TENANT_INFO = {
  ...
  "牛2-31": { name: "來得生物科技有限公司", taxId: "42792378", contactName: "黃秀會", idNo: "F201234945", phone: "07-251-2423／0980-850-787", leaseStart: "2025-12-01", leaseEnd: "2028-11-30", rentUntaxed: 38000, rent: 38000, deposit: 0, dueDay: 10, payBank: "農會", payCompany: "個人戶·趙貴美", rentSchedule: [
    { from: "2025-12-01", untaxed: 38000 },
    { from: "2027-12-01", untaxed: 42000 }
  ], note: "57巷1弄31號。114/12/1～116/11/30 未稅 $38,000；116/12/1～117/11/30 未稅 $42,000。每月10日匯入。所得稅10%及二代健保2.11%乙方負擔。發票依合約期間自動帶入未稅。" },
  "牛2-33": { name: "謝淑伃", taxId: "", contactName: "", idNo: "E222202083", phone: "0973-788-733", leaseStart: "2026-01-01", leaseEnd: "2026-12-31", rentUntaxed: 35500, rent: 35500, deposit: 70000, dueDay: 1, payBank: "現金", payCompany: "現金(保險箱)", note: "57巷1弄33號。個人戶。未稅 $35,500（扣繳 $3,550＋健保 $749 乙方自付）。每月1日現金交給趙浩鈞。" },
  ...
  "拉皮-1A": { name: "南溢製鞋股份有限公司", taxId: "81265944", contactName: "徐志逢", phone: "0910-700-069", leaseStart: "2025-12-01", leaseEnd: "2029-11-30", rentUntaxed: 37000, rent: 38850, dueDay: 1, payBank: "聯邦", payCompany: "統潔", invoiceAddr: "802 高雄市苓雅區永泰路115號", rentSchedule: [
  ...
  "大樹-18": { name: "廣永隆生物科技有限公司", taxId: "90553919", contactName: "陳逸峯", phone: "0939-153-975", leaseStart: "2026-09-01", leaseEnd: "2031-05-31", rentUntaxed: 46000, rent: 48300, dueDay: 1, payBank: "聯邦", payCompany: "統潔", rentSchedule: [
    ...
    { from: "2029-09-01", untaxed: 49000 },
    { from: "2030-09-01", untaxed: 50000 }
  ], note: "九曲路5巷32弄18號。116/9/1 未稅 $47,000；117/9/1 $48,000；118/9/1 $49,000；119/9/1 $50,000。此次合約未附，先沿用租金表。" },
  ...
  FACTORY_GROUPS.forEach(g => {
    g.items.forEach(item => {
      ...
      out.push({
        ...
        deposit: info && info.deposit ? info.deposit : 0,
  ...
  factoryRooms().forEach(r => {
    ...
    if (info && info.name) {
      ...
      tenants.push({
        ...
        contactName: info.contactName || "", idNo: info.idNo || "", address: info.invoiceAddr || "",
        emergencyName: "", emergencyPhone: "", roomId: r.id,
        leaseStart: info.leaseStart || "", leaseEnd: info.leaseEnd || "",
        dueDay: info.dueDay || 1, paid: false, note: info.note || "", rentUntaxed: info.rentUntaxed || 0,
        payBank: info.payBank || "", payCompany: info.payCompany || "", deposit: info.deposit || 0
      });
    }
    rooms.push(r);
  });
...
const JULY115_BOOKS = [
  ["2026-07-02", "in", 46000, "現金(保險箱)", "租金收入　文21（牛2）謝帛翰"],
  ...
  ["2026-07-06", "in", 55000, "現金(保險箱)", "租金收入　文23（牛2）洪耀鴻"],
  ["2026-07-06", "in", 22000, "現金(保險箱)", "租金收入　鳳97-65B（牛3）蔡聖鴻"],
  ["2026-07-08", "in", 45000, "現金(保險箱)", "租金收入　鳳93-2A（拉皮）咘然居"],
  ["2026-07-08", "in", 21690, "現金(保險箱)", "電費收入　93-2A　115/4/29～5/27"],
  ["2026-07-09", "out", 200000, "現金(保險箱)", "個人支出　趙海成存聯邦"],
  ["2026-07-09", "out", 200000, "現金(保險箱)", "個人支出　趙正賢存聯邦"],
  ["2026-07-09", "in", 120000, "現金(保險箱)", "其他收入　文59繳交115/1、2月欠租（張哲嘉）"],
  ["2026-07-09", "out", 10000, "現金(保險箱)", "清潔費支出　牛10（5/15、5/26、6/1、6/8、6/23）"],
  ["2026-07-09", "out", 1000, "現金(保險箱)", "清潔費支出　成$500、賢$500（垃圾桶）"],
  ["2026-07-09", "in", 13221, "現金(保險箱)", "其他收入　牛5垃圾桶（原表寫清潔費支出、金額在其他收入）"],
  ["2026-07-10", "in", 46000, "現金(保險箱)", "租金收入　大樹"],
  ["2026-07-16", "in", 1800, "現金(保險箱)", "水費收入　文東57巷17弄（菊春廠房）"],
  ...
  ["2026-07-16", "out", 28861, "統潔", "水費　93-2 $23002、93-63 $71、$5788", "聯邦"],
  ["2026-07-16", "out", 106479, "統潔", "電費　93-1", "聯邦"],
  ["2026-07-16", "out", 58830, "統潔", "4戶烤漆門　牛10", "聯邦"],
  ["2026-07-23", "out", 11090, "統潔", "電費　楠梓", "聯邦"],
  ["2026-07-23", "out", 295, "統潔", "電話費", "聯邦"],
  ["2026-07-23", "out", 17774, "統潔", "太陽能險　文59、61　文57巷2、6、8", "聯邦"],
  ["2026-07-27", "out", 8558, "統潔", "工程款　支存", "聯邦"],
  ["2026-07-30", "out", 14979, "統潔", "勞保費", "聯邦"],
  ["2026-07-30", "out", 8840, "統潔", "健保費", "聯邦"],
  ["2026-07-30", "out", 7860, "統潔", "勞退金", "聯邦"],

  ["2026-07-01", "in", 14000, "統潔", "牛10　7232 林紜亦　原日期115/6/27", "農會"],
  ["2026-07-01", "out", 14030, "統潔", "牛10　黃明媛　退押金　原日期115/6/29", "農會"],
  ["2026-07-01", "in", 9000, "統潔", "牛10　7041 劉恩彤　原日期115/6/30", "農會"],
  ["2026-07-01", "in", 9000, "統潔", "牛10　7031 朱甫晟　原日期115/6/30", "農會"],
  ["2026-07-01", "in", 7000, "統潔", "牛10　6822 吳孟書黃莉晏　原日期115/6/30", "農會"],
  ["2026-07-01", "in", 7000, "統潔", "牛10　7221 張智傑", "農會"],
  ["2026-07-01", "in", 9000, "統潔", "牛10　6831 吳昱瑋", "農會"],
  ["2026-07-02", "in", 7000, "統潔", "牛10　7021 陳信安", "農會"],
  ...
  ["2026-07-06", "in", 12000, "統潔", "牛10　7032 楊旻憲", "農會"],
  ...
  ["2026-07-15", "in", 9000, "統潔", "牛10　7641 洪子軒", "農會"],
  ["2026-07-16", "in", 9000, "統潔", "牛10　7631 蔡文銘", "農會"],
  ["2026-07-16", "out", 4443, "統潔", "牛10　水費", "農會"],
  ["2026-07-16", "out", 1616, "統潔", "牛10　火險", "農會"],
  ["2026-07-17", "in", 14000, "統潔", "牛10　7632 謝佩君", "農會"],
  ["2026-07-20", "in", 18000, "統潔", "牛10　7231 林科承朱宣羽　6、7月", "農會"],
  ...
  ["2026-07-01", "in", 110250, "信潔", "牛6　93-60 鈺晟實業　六月份", "聯邦"],
  ["2026-07-06", "in", 42000, "信潔", "牛5　97-66 旭瑞食品　票號1979471", "聯邦"],
  ["2026-07-07", "in", 325500, "信潔", "牛6　93-55 誠家食品　七、八月份", "聯邦"],
  ["2026-07-08", "in", 50700, "信潔", "牛6　59 黃儒清　應收50400＋補300", "聯邦"],
  ["2026-07-14", "out", 21528, "信潔", "水費支出", "聯邦"],
  ["2026-07-09", "out", 13912, "信潔", "垃圾清運支出", "聯邦"],
  ["2026-07-10", "in", 105000, "信潔", "牛6　62 驊勝食品", "聯邦"],
  ["2026-07-13", "in", 38000, "信潔", "牛5　97-76 陳雅琪", "聯邦"],
  ["2026-07-08", "out", 23121, "信潔", "利息支出", "聯邦"],
  ["2026-07-14", "out", 95839, "信潔", "營業稅支出", "聯邦"],
  ["2026-07-15", "in", 42000, "信潔", "牛5　97-75 力胤", "聯邦"],
  ["2026-07-16", "in", 42000, "信潔", "牛5　97-73 映升", "聯邦"],
  ["2026-07-21", "in", 42000, "信潔", "牛5　97-71 莊記綠豆鑽", "聯邦"],
  ["2026-07-01", "in", 44100, "信潔", "牛5　97-72 萬事盈", "聯邦"],
  ["2026-07-22", "in", 42000, "信潔", "牛5　97-67 弘翔音響", "聯邦"],
  ["2026-07-28", "in", 42000, "信潔", "牛5　97-68 樂芯食品　票號3872684", "聯邦"],
  ["2026-07-31", "in", 110250, "信潔", "牛6　93-60 鈺晟實業　七月份", "聯邦"],

  ["2026-07-01", "in", 28302, "個人戶·趙文榮", "薪資／伙食"],
  ["2026-07-31", "in", 61055, "個人戶·趙文榮", "退綜所稅"],
  ["2026-07-01", "in", 45000, "個人戶·趙洪漳", "租金　文東61號　林志雄"],
  ["2026-07-31", "in", 33706, "個人戶·趙洪漳", "退綜所稅"],
  ["2026-07-20", "in", 35500, "個人戶·趙浩鈞", "租金　文東33號　謝淑伃"],
  ["2026-07-31", "in", 24517, "個人戶·趙浩鈞", "退綜所稅"],
  ["2026-07-10", "in", 70000, "個人戶·趙文彬", "租金　57巷2、6號　丞億科技（與浩鈞支票）"],
  ["2026-07-31", "in", 25599, "個人戶·趙文彬", "退綜所稅"],
  ["2026-07-10", "in", 36000, "個人戶·趙苡真", "租金　57巷8號　欣上宜"],
  ["2026-07-15", "in", 70312, "個人戶·趙苡真", "租金　文東35號　詠利實業　七、八月份"],
  ["2026-07-01", "in", 40800, "個人戶·趙海成、趙正賢", "租金　鳳仁63號　卓建忠　原日期115/6/30　房租40000＋貨櫃800"],
  ["2026-07-03", "in", 12000, "個人戶·趙海成、趙正賢", "租金　鳳仁65A　羅美芳　六月份遲繳"],
  ["2026-07-03", "in", 28772, "個人戶·江秀霞", "薪資"],
  ["2026-07-03", "in", 28772, "個人戶·黃思敏", "薪資"]
];
...
function roomNoFromBookNote(note) {
  ...
  if (m10) return m10[1];
  const rules = [
    [/牛7|93-63\s*1F|驊勝.*1F/, "牛7-1F"],
    [/93-63\s*2F/, "牛7-2F"],
    [/牛8|97-77|97-78|錦芳/, "牛8-77"],
    [/93-1A|拉皮-1A|南溢/, "拉皮-1A"],
    [/93-1B|拉皮-1B|鈺晟.*拉皮|拉皮.*鈺晟/, "拉皮-1B"],
    [/93-2A|拉皮-2A|咘然|孫梅芳/, "拉皮-2A"],
...
function applyJuly115Books(data) {
  ...
  if (data.booksImportVer === BOOKS_IMPORT_VER && hasJuly && hasOpen) {
    data.books.forEach(b => {
      if (b && b.importTag === "july115") {
        const no = roomNoFromBookNote(b.note);
        if (no) b.roomNo = no;
      }
    });
  ...
  JULY115_BOOKS.forEach((row, i) => {
    data.books.push({
      ...
      note: row[4],
      bank: row[5] || "",
      roomNo: row[6] || roomNoFromBookNote(row[4]),
      importTag: "july115",
...
function applyOfficeSubsidyTenant(data) {
  ...
  const info = TENANT_INFO["7651"] || {};
  let t = (data.tenants || []).find(x => x && (x.id === "t7651" || (x.roomId === room.id && !x.former && !x.demo && !x.incoming)));
  if (!t) {
    t = { id: "t7651", roomId: room.id, dueDay: 1, paid: false, paidYm: payYmNow(), paidTouched: true };
    data.tenants.push(t);
  }
  room.tenantId = t.id;
  t.name = info.name || "吳慧青";
  t.subsidy = true;
...
function applyFactoryRoster(data) {
  factoryRooms().forEach(seedRoom => {
    ...
    if (!room) {
      ...
      room.group = seedRoom.group || room.group;
      room.street = seedRoom.street || room.street;
      room.company = seedRoom.company || room.company;
      room.location = seedRoom.location || room.location;
      room.manager = seedRoom.manager || room.manager;
      room.kind = "factory";
    ...
    if (!info || !info.name) {
      if (t && String(t.id || "").indexOf("tf-") === 0) {
        t.former = true;
        t.leftOn = t.leftOn || nowStamp();
      }
    ...
    if (!t) {
      t = { id: "tf-" + room.no, roomId: room.id, dueDay: info.dueDay || 1, paid: false, idNo: "", address: "", emergencyName: "", emergencyPhone: "" };
      data.tenants.push(t);
    }
    t.name = info.name;
    t.former = false;
    ...
    t.taxId = info.taxId || "";
    t.contactName = info.contactName || t.contactName || "";
    if (info.idNo) t.idNo = info.idNo;
    if (info.leaseStart) t.leaseStart = info.leaseStart;
    if (info.leaseEnd) t.leaseEnd = info.leaseEnd;
    if (info.note) t.note = info.note;
...
function rocYmd(y, m, d) {
  y = Number(y); m = Number(m); d = Number(d);
  if (!y || !m || !d) return "";
  if (y < 1911) y += 1911;
  return y + "-" + String(m).padStart(2, "0") + "-" + String(d).padStart(2, "0");
}
function parseRentScheduleFromNote(note, leaseStart, baseUntaxed) {
  const text = String(note || "");
  if (!/未稅/.test(text)) return null;
  const map = new Map();
  const add = (y, mo, da, amt) => {
    const from = rocYmd(y, mo, da);
    const untaxed = Number(String(amt || "").replace(/,/g, ""));
    if (!from || !untaxed) return;
    map.set(from, { from, untaxed });
  };
  const re = /(\d{2,3})\/(\d{1,2})\/(\d{1,2})(?:\s*[～~至\-]+\s*\d{2,3}\/\d{1,2}\/\d{1,2})?\s*(起)?\s*(未稅)?\s*\$?\s*([\d,]{4,})/g;
  let m;
  while ((m = re.exec(text))) {
    const tag = text.slice(m.index, m.index + 28);
    if (/押金|入押|入\s*\$/.test(tag)) continue;
    if (m[4] || m[5] || map.size) add(m[1], m[2], m[3], m[6]);
  }
  const hits = [...map.values()].sort((a, b) => a.from.localeCompare(b.from));
  if (!hits.length) return null;
  const start = String(leaseStart || "").slice(0, 10);
  const base = Math.round(Number(baseUntaxed) || 0);
  if (start && base && hits[0].from > start) hits.unshift({ from: start, untaxed: base });
  return hits;
}
function pickRentSchedule(t, r, ymd) {
  const no = String((r && r.no) || "");
  const info = FACTORY_TENANT_INFO[no] || {};
  const parsed = parseRentScheduleFromNote((t && t.note) || info.note, (t && t.leaseStart) || info.leaseStart, (t && t.rentUntaxed) || info.rentUntaxed);
  const sched = parsed || (t && t.rentSchedule) || info.rentSchedule;
...
function applyRentSchedules(data) {
  ...
  (data.tenants || []).forEach(t => {
    ...
    if (!hit) return;
    const parsed = parseRentScheduleFromNote(t.note || (FACTORY_TENANT_INFO[room && room.no] || {}).note, t.leaseStart, t.rentUntaxed);
    if (parsed && parsed.length) t.rentSchedule = parsed;
...
function applyTenantRoster(data) {
  const fillStudio = (no, allowOffice) => {
    ...
    if (!room) return;
    if (room.status === "office" && !allowOffice) return;
    const bld = STUDIO_BUILDINGS.find(b => b.prefix === studioPrefix(no));
    if (bld) {
    ...
    if (info.name) {
      let t = data.tenants.find(x => x.roomId === room.id && !x.former && !x.demo && !x.incoming && !x.loginRevoked);
      const keepNew = t && (isMoveInTenant(t) || (t.name && !sameTenantName(t.name, info.name)));
      if (keepNew) {
        room.tenantId = t.id;
      ...
      t.placeholder = false;
      if (info.phone) t.phone = info.phone;
      if (info.contactName) t.contactName = info.contactName;
      if (info.leaseStart) t.leaseStart = info.leaseStart;
      if (info.leaseEnd) t.leaseEnd = info.leaseEnd;
      ...
      const listed = info.rent != null ? info.rent : studioRentOf(no);
      if (listed != null) room.rent = listed;
      if (info.deposit != null) room.deposit = info.deposit;
      else if (listed > 0 && (room.deposit == null || room.deposit === "")) room.deposit = listed * 2;
      room.tenantId = t.id;
      if (room.status !== "repair" && room.status !== "office") room.status = "rented";
      }
    } else {
...
function ensureStudioTenant(data, no) {
  if (!data) return;
  const info = TENANT_INFO[String(no || "")];
  if (!info || !info.name) return;
  if (!Array.isArray(data.rooms)) data.rooms = [];
  if (!Array.isArray(data.tenants)) data.tenants = [];
  let room = data.rooms.find(r => String(r.no) === String(no));
  if (!room) {
    const seedRoom = (SEED && SEED.rooms || []).find(r => String(r.no) === String(no));
    if (seedRoom) {
      room = structuredClone(seedRoom);
      data.rooms.push(room);
    }
  ...
  if (!room) {
    room = { id: "r" + no, no: String(no), title: "套房", kind: "studio", status: "rented", rent: studioRentOf(no) || 7000, deposit: 14000 };
    data.rooms.push(room);
  }
  room.no = String(no);
...
function hidePracticeFormers(data) {
  ...
  const latestByRoom = {};
  (data.tenants || []).forEach(t => {
    if (!t || !t.former || t.cancelledApply || t.demo || t.placeholder) return;
    const r = (data.rooms || []).find(x => x && x.id === t.roomId);
    if (!isPracticeStudioNo(r && r.no)) return;
    const cur = latestByRoom[t.roomId];
    if (!cur || String(t.leftOn || "") > String(cur.leftOn || "")) latestByRoom[t.roomId] = t;
  });
  (data.tenants || []).forEach(t => {
    if (!t) return;
    const r = (data.rooms || []).find(x => x && x.id === t.roomId);
    if (isPracticeStudioNo(r && r.no) && t.former && !t.cancelledApply && !t.demo) {
      const keep = latestByRoom[t.roomId] && latestByRoom[t.roomId].id === t.id;
      if (keep) {
        if (t.practiceStay) { t.practiceStay = false; t.edited = true; t.editedAt = Date.now(); dirty = true; }
      } else if (!sameTenantName(t.name, "小芬") && !t.practiceStay) {
        t.practiceStay = true;
        t.edited = true;
        t.editedAt = Date.now();
        dirty = true;
    ...
    if (isPracticeStudioNo(r && r.no) && !t.former && !t.incoming && !t.prospect && !t.placeholder) {
      const current = r && r.tenantId && t.id === r.tenantId && r.status !== "vacant";
...
function persistLedger(data) {
  ...
  if (extraGone.length) {
    gone.push(...extraGone);
    books = books.filter(b => extraGone.indexOf(b.id) < 0);
  }
  const merged = {
...
function scrollAiLogEnd() {
  const pane = document.getElementById("ai-log");
  if (!pane) return;
  pane.scrollTop = pane.scrollHeight;
  const last = pane.querySelector(".ai-row:last-child");
...
function persistGatePage() {
  if (!ui || ui.role) {
    try { sessionStorage.removeItem("tj-gate-page"); } catch {}
    return;
  }
  try { sessionStorage.setItem("tj-gate-page", ui.page || "home"); } catch {}
}
function readGatePage() {
  try { return String(sessionStorage.getItem("tj-gate-page") || ""); } catch { return ""; }
}
function isGatePage(p) {
  return /^(tenant-login|admin-login|move-in|tenant-forgot|tenant-setpass)$/.test(String(p || ""));
}
function restoreUi() {
  try {
    const stayingGate = !ui.role && isGatePage(ui.page);
    const s = readUiSnap();
    if (stayingGate && (!s || !s.role)) return;
    if (!s) {
      if (!ui.role) {
        const g = readGatePage();
        if (isGatePage(g)) ui.page = g;
      }
      return;
    }
    ui.role = s.role;
    ...
    if (s.role === "tenant") {
      ...
      if (t && t.incoming && !isDemoTenant(t)) {
        if (s.prospectPreview && t.prospect) {
          ui.prospectPreview = true;
          ui.role = "tenant";
          ui.tenantId = t.id;
          if (room) { ui.roomId = room.id; ui.roomNo = room.no; }
          if (!ui.page || ui.page === "dash" || ui.page === "tenant-login") ui.page = "home";
          persistUi();
          return;
        }
        ui.role = null;
...
const INTRO_SRC = "images/intro-city.mp4?v=0162";
const INTRO_POSTER = "images/intro-city.jpg?v=0162";
...
function ensureIntro() {
  ...
  document.body.insertAdjacentHTML("beforeend", `<div class="intro-mask" id="intro-mask">
    <video id="intro-video" autoplay muted playsinline webkit-playsinline preload="auto" poster="${INTRO_POSTER}">
...
function liveInvoiceNote(r, t) {
  ...
  if (/^\d/.test(head) && head.length <= 24) {
    return head.replace(/號$/g, "").replace(/(\d+)之(\d+[A-Za-z]?)/g, "$1-$2");
  }
...
function invoiceCopyHtml(r, t, copyName) {
  ...
  if (triple) {
    ...
    return `<section class="inv-photo triple">
      ...
      ${boxes}
      ${at(W, H, 1034, 232, 78, 32, "invoiceY", y)}
      ${at(W, H, 1160, 232, 68, 32, "invoiceMo", mo)}
      ${at(W, H, 1270, 232, 72, 32, "invoiceDay", day)}
  ...
  return `<section class="inv-photo double">
    ...
    ${at(W, H, 301, 204, 1440, 42, "invoiceBuyer", buyer, "inv-left inv-buyer")}
...
function isFactoryAutoRentBook(b, data) {
  if (!b) return false;
  const id = String(b.id || "");
  if (/bk-j115-|bk-a31-|bk-yusheng/.test(id)) return false;
  const note = String(b.note || "").replace(/\s+/g, " ").trim();
  const room = (data && data.rooms || []).find(r => r && String(r.no) === String(b.roomNo || ""));
  ...
  if (/富強鑫|凱薩琳/.test(note) && /租金/.test(note)) return true;
  if (roomIsFactory(room) && /租金/.test(note) && (isRentAutoBook(b) || ymdOf(b.date).slice(-2) === "01")) return true;
  return factoryTenantNames(data).some(n => n && note.indexOf(n) >= 0 && /租金/.test(note) && (isRentAutoBook(b) || ymdOf(b.date).slice(-2) === "01"));
}
function dropFactoryRentAutos(data) {
  if (!data) return 0;
  const extra = loadLedgerBackup();
  const pool = [].concat(data.books || [], (extra && extra.books) || []);
  const goneIds = [...new Set(pool.filter(b => isFactoryAutoRentBook(b, data)).map(b => b && b.id).filter(Boolean))];
...
function syncPaidRentBooks(data) {
  ...
  const ym = payYmNow();
  const keep = new Set();
  const factoryRooms = new Set((data.rooms || []).filter(roomIsFactory).map(r => r && r.id));
  (data.tenants || []).forEach(t => {
    const room = (data.rooms || []).find(r => r && r.id === t.roomId);
    if (paidThisMonth(t) && room && !roomIsFactory(room) && !room.demo) {
  ...
  data.books = (data.books || []).filter(b => {
    ...
    const room = (data.rooms || []).find(r => r && (String(r.no) === String(b.roomNo || "") || r.id === b.roomId));
    const tid = b.linkedTenantId || ((String(b.id || "").match(/^bk-rent-(.+)-\d{4}-\d{2}$/) || [])[1] || "");
    const tenant = (data.tenants || []).find(t => t && t.id === tid);
...
function toggleTenantPaid(id) {
  ...
  if (!t) {
    const room = (state.rooms || []).find(r => r && (String(r.id) === sid || String(r.no) === sid));
...
function ensureDemoTenant(data) {
  ...
  if (!room) {
    ...
    if (typeof ensurePhotos === "function") ensurePhotos(room);
  }
  let t = data.tenants.find(x => x.id === "t-demo" || x.roomId === room.id);
  if (!t) {
    t = {
      id: "t-demo", name: "開發者測試", roomId: room.id, paid: false, demo: true,
      leaseStart: oneYearLeaseRange(todayYmd()).start, leaseEnd: oneYearLeaseRange(todayYmd()).end, dueDay: 1,
    ...
    if (!t.leaseStart) t.leaseStart = oneYearLeaseRange(todayYmd()).start;
    if (!t.leaseEnd) t.leaseEnd = oneYearLeaseRange(todayYmd()).end;
...
function resetDemoCycle(kind) {
  ...
  const yr = oneYearLeaseRange(todayYmd());
  t.leaseStart = yr.start;
  t.leaseEnd = yr.end;
  t.dueDay = 1;
  t.signAppointAt = "";
  t.signRoomId = "";
...
function openInvoiceForRoom(roomId, from) {
  ...
  const t = r ? invoiceTenantOf(r) : null;
  if (r && t && !roomIsFactory(r) && !leaseCoversYm(t, r, payYmNow())) {
    const start = tenantOccupancyStart(t, r) || t.leaseStart || "";
    toast("合約 " + (start || "") + " 才開始，本月不開立發票");
    return;
  }
  ui.invoiceRoomId = roomId;
  ui.invoiceFrom = from || "tenants";
...
function invoiceOverviewRows(kind) {
  ...
  (state.tenants || []).forEach(t => {
    if (!t) return;
    const room = (state.rooms || []).find(r => r && r.id === t.roomId);
    if (!studioInvoiceEligible(t, room)) return;
  ...
  STUDIO_NOS.concat(["7651"]).forEach(no => {
    ...
    const live = (state.tenants || []).find(x => x && x.roomId === room.id && !x.former && !x.demo && String(x.name || "").trim());
    if (live) {
      if (!studioInvoiceEligible(live, room)) return;
      seen.add(String(no));
      rows.push(studioInvoiceRow(no, room, live, info));
      return;
    }
    if (room.status === "vacant") return;
    seen.add(String(no));
    rows.push(studioInvoiceRow(no, room, null, info));
  });
  rows.sort((a, b) => {
    const da = a.remitYmd || a.invoiceYmd || "";
    const db = b.remitYmd || b.invoiceYmd || "";
    if (da && db && da !== db) return da.localeCompare(db);
    if ((a.remitYmd || a.invoiceYmd) && !(b.remitYmd || b.invoiceYmd)) return -1;
    if (!(a.remitYmd || a.invoiceYmd) && (b.remitYmd || b.invoiceYmd)) return 1;
    return String(a.room).localeCompare(String(b.room), "zh-Hant");
  });
  return rows;
}
function factoryInvoiceOverviewRows() {
  ...
  entries.forEach(entry => {
    ...
    const amount = rooms.reduce((n, r) => n + (Number(r.rent) || Number((tenants.find(x => x.roomId === r.id) || {}).rent) || 0), 0);
...
function leaseCoversYm(t, r, ym) {
  ...
  if (leasePartForYm(t, r, y)) return true;
  if (start && String(start).slice(0, 7) <= y && (!end || String(end).slice(0, 7) >= y)) return true;
  return false;
}
...
function thisMonthRentLineHtml(t, r) {
  ...
  if (!leaseCoversYm(t, r, ym)) return escapeHtml("本月租金");
...
function roomCurrentTenant(room) {
  if (!room) return null;
...
function studioContractRent(t, r) {
  ...
  const n = Number((r && r.rent) || (t && t.rent) || 0) || 0;
  return n;
}
function signSlotKey(at) {
  return String(at || "").replace(" ", "T").slice(0, 13);
}
...
function nextSignSlots(n, keep, minYmd, onlyDay, maxYmd) {
  ...
  for (let i = 0; i < span && out.length < need; i++) {
    ...
    hours.forEach(h => {
      ...
      if (slot.getTime() < now.getTime() + 20 * 60000) return;
      const key = ymd + "T" + p(h) + ":00";
      if (busy.has(signSlotKey(key))) return;
      out.push(key);
    });
  }
...
function signCalHtml(minYmd, selectedYmd, keepAt, maxFastYmd) {
  ...
  return `<div class="sign-cal-nav">
      ...
      <button type="button" class="ghost" data-sign-cal-nav="1">下一月</button>
    </div>
    <div class="cal-grid sign-cal">
      ${["日", "一", "二", "三", "四", "五", "六"].map(w => `<div class="cal-w">${w}</div>`).join("")}
      ${cells.map(d => {
        if (!d) return `<div class="cal-cell empty"></div>`;
        const ymd = y + "-" + p(m) + "-" + p(d);
        const wd = new Date(y, m - 1, d).getDay();
        const slots = nextSignSlots(1, keepAt, min, ymd);
        const off = ymd < min || wd === 0 || !slots.length;
        const on = ymd === sel;
        const fast = ymd === fastDay;
        return `<button type="button" class="cal-cell${on ? " on" : ""}${off ? " off" : ""}${fast ? " fast" : ""}" data-sign-cal-day="${ymd}" ${off ? "disabled" : ""}>
          <em>${d}</em>
          ${fast && !off ? `<span class="mark">最快</span>` : ""}
        </button>`;
      }).join("")}
    </div>`;
}
...
function bankPeriodBits(acct, start, end) {
  ...
  const all = collectLedger().filter(x => rowAccount(x) === acct);
  const period = all.filter(x => x.date >= start && x.date <= end);
  const hist = all.filter(x => x.date <= end);
  return banks.map(b => {
    const hit = period.filter(x => rowBank(x) === b);
    const past = hist.filter(x => rowBank(x) === b);
    const inn = hit.filter(x => x.type === "in").reduce((s, x) => s + x.amount, 0);
    const out = hit.filter(x => x.type === "out").reduce((s, x) => s + x.amount, 0);
    const ledger = past.filter(x => x.type === "in").reduce((s, x) => s + x.amount, 0)
      - past.filter(x => x.type === "out").reduce((s, x) => s + x.amount, 0);
    const opening = Number((state.accountOpenings || {})[acct + "·" + b]) || 0;
    return { name: b, inn, out, bal: ledger + opening };
  });
}
  ...
  return `<div class="small">${rangeLabel}${extra || ""}</div>
    ${selected.length ? selected.map(x => `
      <div class="led-line clickable" data-edit-led="${x.id}" data-edit-src="${x.source}">
        <div class="led-head">
          <b>${x.type === "memo"
          ? `<span class="led-trip">行程</span> · ${escapeHtml(x.note || "記下的事")}`
...
function inferOneFile(file) {
  ...
  const meta = guessMetaFromName(blob);
  const date = meta.date || (file && file.lastModified
    ? (() => { const d = new Date(file.lastModified); return d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0"); })()
    : ymdOf(nowStamp()));
  const amount = meta.amount || (party && !bill && party.rent) || 0;
  const bank = guessBank(blob) || meta.bank || "";
  let title = "現場紀錄";
  ...
  if (isDepositRefund(blob)) {
    ...
    title = "超商";
    place = "超商";
  } else if (/存摺|對帳|簿子|入帳|銀行|聯邦|兆豐|農會/.test(blob) || bank) {
    title = "跑銀行／入帳";
    place = place || "";
  ...
  return {
    ...
    note: [bill && bill.name, party && party.name, blob].filter(Boolean).join(" · "),
    pendingBank, party, bill, files: blob, fileName: blob, cashType,
    needCompany: !company,
...
function inferFromUpload(files) {
  const list = [...(files || [])].filter(f => !isSheetFile(f)).map(inferOneFile);
  return list.length === 1 ? list[0] : { items: list, title: list.length + " 張照片" };
}
function errandGuessList() {
  if (Array.isArray(ui.errandGuesses) && ui.errandGuesses.length) return ui.errandGuesses;
  if (ui.errandGuess && ui.errandGuess.items) return ui.errandGuess.items;
  if (ui.errandGuess) return [ui.errandGuess];
  return [];
}
function errandGuessHtml(g) {
  ...
  return `<div id="errand-guess-box">${list.map((item, i) => {
    const bits = [
      ...
      item.date, item.title, item.place, item.company,
      item.amount ? money(item.amount) : "",
      item.cashType === "out" ? "支出" : "",
      item.pendingBank ? "先入現金，待存銀行" : ""
    ].filter(Boolean);
...
function refreshErrandGuessBox() {
  ...
  if (!box) return;
  const html = errandGuessHtml(errandGuessList());
...
function bindErrandGuessPicks() {
  document.querySelectorAll("[data-guess-co]").forEach(btn => {
    btn.onclick = e => {
      ...
      const i = Number(btn.dataset.guessI) || 0;
      if (!list[i]) return;
      list[i].company = btn.dataset.guessCo;
      list[i].needCompany = false;
...
function nudgePayOne(t, silent) {
  ...
  const rent = room ? money(thisMonthRentOf(t, room)) : "";
...
function addIncomingTenant(roomId, fields) {
  ...
  const range = continueLeaseRange(r, { incoming: true, roomId: r.id });
  const t = {
    id: "tin" + Date.now(),
    name,
    ...
    leaseStart: fields.leaseStart || range.start,
    leaseEnd: fields.leaseEnd || range.end,
...
function submitMoveIn() {
  ...
  const t = addIncomingTenant(room.id, {
    ...
    rent: studioContractRent(null, room)
  });
  if (!t) { toast("送出失敗，請再試一次"); return; }
  t.prospect = true;
  t.applyPending = true;
  t.applyUnread = true;
  t.applyAt = nowStamp();
...
function moveInView() {
  ...
  if (r && !daySlots.length) {
    const nxt = nextSignSlots(1, d.signAppointAt, win.min, "", win.maxFast)[0] || nextSignSlots(1, d.signAppointAt, win.min)[0];
    if (nxt) day = ymdOf(nxt);
    daySlots = nextSignSlots(8, d.signAppointAt, win.min, day);
  }
  if (r && (!d.signAppointAt || ymdOf(d.signAppointAt) !== day) && daySlots[0]) d.signAppointAt = daySlots[0];
...
function gateView() {
  if (ui.page === "move-in") return moveInView();
  ...
  return `<div class="topbar"><div class="slide-right"><div class="eyebrow">LEASE</div><h1>租約</h1></div></div>
    <div class="screen">
      ...
      ${t.leaseEnd ? `<p class="small slide-left" style="margin-top:12px;padding:0 6px">${(() => {
        ...
        return `現有租客請於到期前 30 天內確定是否續約${by ? "（" + by + " 前）" : ""}。仲介帶新客最快可在到期前 15 天簽約。`;
      })()}</p>` : ""}
      ${isDemoTenant(t) || isDemoRoom(r) ? demoResetBarHtml() : ""}
      ${(() => {
...
function bindAdminPageSwipe() {
  ...
  sc.addEventListener("touchstart", e => {
    ...
    if (e.target.closest(".swipe-wrap, .cal-grid, .seg, .tenant-search, .rev-card, .rev-sheet, .rev-zoom, #rev-zoom, form, input, textarea, select, .tabs, label, .ai-block, .card, button, .work-scroll")) return;
...
const DEFAULT_LUNCH_SPOTS = [
  ...
  },
  {
    ...
    note: "團體便當可預訂",
    photos: ["images/food/yakiniku.jpg?v=0220"]
  },
  {
    ...
    note: "文龍東路，可電話預訂",
    photos: ["images/food/macc.jpg?v=0209"]
  },
  {
  ...
  },
  {
    ...
    menu: ["自助配菜便當", "雞腿飯", "排骨飯"],
    note: "海洋一路，菜色依當日",
    photos: ["images/food/jinyuanshan.jpg?v=0224"]
  },
  {
  ...
  },
  {
    ...
    addr: "高雄市鳳山區", phone: "07-743-7283",
    hours: "",
    menu: ["鍋燒意麵", "鮮蝦麵", "燒肉飯"],
    note: "訂購專線見菜單",
    photos: ["images/food/haoxifu.jpg?v=0226"]
  },
  {
  ...
  },
  {
    ...
    menu: ["蛋奶素簡餐", "蔬食便當", "咖啡"],
    note: "同路文龍東路 745 號",
    photos: ["images/food/nikkovege.jpg?v=0228"]
  }
];
...
function adminSettings() {
  ...
  return `<div class="admin-grid list settings-stack">
    ...
    <div class="card card-body">
      ...
      <button type="button" class="ghost" id="force-reload-app" style="margin-top:8px">重新載入這個裝置</button>
    </div>
    <div class="card card-body">
  ...
  if (ball) {
    ...
    const onEnd = e => {
      ...
      if (!moved) {
        ...
        ui.errandBallOpen = !ui.errandBallOpen;
        setTimeout(() => ensureErrandBall(), 120);
      } else {
...
function tenantListOfKind(kind, opts) {
  ...
  list.forEach(t => {
    const r = state.rooms.find(x => x.id === t.roomId);
    const key = factory
      ? "f:" + String(t.taxId || t.name || t.id) + "|" + factoryAddress(r)
      : (r ? "n:" + String(r.no || r.id) : "t:" + t.id);
    if (seenRoom.has(key)) return;
    seenRoom.add(key);
    if (!factory) {
      const info = r && TENANT_INFO[String(r.no || "")];
      if (info && info.name && !isMoveInTenant(t) && !t.incoming && (!t.name || sameTenantName(t.name, info.name))) t.name = info.name;
    }
    uniq.push(t);
  });
...
function tenantEntriesOfKind(kind, opts) {
  ...
  list.forEach(t => {
    ...
    const e = map.get(key);
    if (!e.tenants.some(x => x.id === t.id)) e.tenants.push(t);
...
function tenantEntryDetailsHtml(kind, entry) {
  ...
  return `${incSelf ? incomingActionHtml(incSelf, r) : ""}
      ...
      ${(() => {
        ...
        const b = es && es.sig2 && String(es.sig2).startsWith("data:") ? es.sig2 : "";
        if (!a && !b) return "";
        return `<div class="esig-row">${a ? `<img src="${a}" alt="簽名" class="esig-thumb">` : ""}${b ? `<img src="${b}" alt="簽名" class="esig-thumb">` : ""}</div>`;
      })()}
...
function bindHandover() {
  ...
  document.querySelectorAll("[data-apply-confirm]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault(); e.stopPropagation();
      const inc = (state.tenants || []).find(x => x.id === btn.dataset.applyConfirm);
      confirmIncomingApply(inc);
      ui.keepScroll = true;
      render();
    };
  });
...
function bindTenantSearch() {
  ...
  document.querySelectorAll("[data-tenant-chip]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      setTenantChip(btn.getAttribute("data-tenant-chip") || "");
      document.querySelectorAll("[data-tenant-chip]").forEach(b => {
        b.classList.toggle("on", tenantChipOn() === b.getAttribute("data-tenant-chip"));
      });
      apply();
    };
  });
...
function openTenantSheet(id) {
  ...
  ui.sheetEnter = true;
  try {
    const rid = String(next).indexOf("vac-") === 0 ? String(next).slice(4) : "";
    const t = (state.tenants || []).find(x => x && x.id === next);
    const room = rid
...
function tenantSheetView() {
  ...
  if (id.indexOf("vac-") === 0) {
    ...
    const enterVac = ui.sheetEnter ? " tenant-sheet-enter" : "";
    ui.sheetEnter = false;
    return `<div class="admin-grid list tenant-sheet-page${enterVac}">
...
function bindTenantListTools() {
  ...
  document.querySelectorAll("#tenant-list [data-invoice]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      openInvoiceForRoom(btn.dataset.invoice, "tenants");
    };
  });
...
function fitGateTitle() {
  const h = document.querySelector(".gate h1.firm-name");
  if (!h) return;
  const max = Math.max(120, (h.parentElement ? h.parentElement.clientWidth : h.clientWidth) || 0);
  h.style.whiteSpace = "nowrap";
  let size = 26;
  h.style.fontSize = size + "px";
  while (size > 13 && h.scrollWidth > max + 1) {
    size -= 0.5;
    h.style.fontSize = size + "px";
  }
}
...
function bindMoveInForm() {
  ...
  if (startEl) {
    startEl.onchange = () => {
      captureMoveInDraft();
      const d = ensureMoveIn();
      const r = (state.rooms || []).find(x => x && x.id === d.roomId);
      const min = r ? roomSoonestStart(r, { incoming: true }) : todayYmd();
      if (!d.leaseStart || d.leaseStart < min) {
        d.leaseStart = min;
        toast("起始日不可早於 " + min);
      }
  ...
  document.querySelectorAll("[data-sign-cal-nav]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      captureMoveInDraft();
      const delta = Number(btn.dataset.signCalNav) || 0;
      let y = ui.signCalYear || new Date().getFullYear();
      let m = ui.signCalMonth || (new Date().getMonth() + 1);
      m += delta;
      if (m < 1) { m = 12; y -= 1; }
      if (m > 12) { m = 1; y += 1; }
      ui.signCalYear = y;
  ...
  document.querySelectorAll("[data-sign-cal-day]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      ...
      const d = ensureMoveIn();
      const r = (state.rooms || []).find(x => x.id === d.roomId);
      const min = signWindow(r, { incoming: true }).min;
  ...
  document.querySelectorAll("[data-sign-slot]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      captureMoveInDraft();
  ...
  const go = document.getElementById("move-submit");
  if (go) go.onclick = e => {
    e.preventDefault();
    captureMoveInDraft();
    askMoveSubmit();
...
function bindSignAgree() {
  ...
  const lab = document.querySelector("label.sign-agree[for='sign-agree']");
  if (!box) return;
...
function bindAdmin() {
  ...
  document.querySelectorAll("[data-invoice]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      openInvoiceForRoom(btn.dataset.invoice, ui.page === "room-edit" ? "room-edit" : "tenants");
    };
  });
  const printInv = document.getElementById("print-invoice");
  if (printInv) printInv.onclick = () => {
    document.querySelectorAll("[data-inv]").forEach(el => { ui[el.dataset.inv] = el.value; });
    const track = document.getElementById("inv-track");
    const num = document.getElementById("inv-num");
    if (track) ui.invoiceTrack = String(track.value || "").trim().toUpperCase();
    if (num) ui.invoiceNum = String(num.value || "").trim();
    ui.keepScroll = true;
    render();
...
function tenantSettings() {
  ...
  return `
    ...
    <div class="screen settings-stack">
      ...
      ${lookSettingsHtml()}
      <div class="card card-body">
        <button type="button" class="ghost" id="logout-set">${ui.devPreview ? "返回後台" : isProspectPreview() ? "離開預覽" : "登出"}</button>
      </div>
    </div>`;
}
...
function bindAdminSettings() {
  ...
  document.querySelectorAll("[data-notify-pref]").forEach(btn => {
    btn.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      ...
      const prefs = loadNotifyPrefs();
      prefs[btn.dataset.notifyPref] = on;
      saveNotifyPrefs(prefs);
      toast(on ? "已開啟此類通知" : "已關閉此類通知");
    };
  });
...
function bindAdminAi() {
  ...
  if (pane) {
    pane.scrollTop = pane.scrollHeight;
    requestAnimationFrame(scrollAiLogEnd);
  }
...
async function boot() {
  ...
  try {
    ...
    persistLedger(state);
    try { connectPaidCloud(); } catch {}
    if (got === "local-newer" || cloudDirty) await pushCloud();
    restoreUi();
    beatPresence();
    render();
